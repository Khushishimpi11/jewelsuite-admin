...
interface InvItem {
  id: number; name: string; sku: string; category: string; brand: string; stock: number; maxStock: number;
...
const inventory: InvItem[] = [
  { id: 1, name: "Diamond Solitaire Ring 18K", sku: "JK-DR-001", category: "Rings", brand: "JewelsKart", stock: 8, maxStock: 20, purchasePrice: "₹1,00,000", sellingPrice: "₹1,45,000", margin: "31.0%", status: "In Stock", description: "Premium 18K gold diamond solitaire ring", image: "💍" },
  { id: 2, name: "Gold Temple Necklace", sku: "JK-GN-002", category: "Necklaces", brand: "Rajdhani Jewels", stock: 3, maxStock: 15, purchasePrice: "₹2,00,000", sellingPrice: "₹2,85,000", margin: "29.8%", status: "Low Stock", description: "Traditional temple design gold necklace", image: "📿" },
  { id: 3, name: "Pearl Drop Earrings", sku: "JK-PE-003", category: "Earrings", brand: "Hyderabad Pearls", stock: 15, maxStock: 25, purchasePrice: "₹18,000", sellingPrice: "₹28,500", margin: "36.8%", status: "In Stock", description: "Elegant pearl drop earrings", image: "✨" },
  { id: 4, name: "Platinum Band Ring", sku: "JK-PR-004", category: "Rings", brand: "Platinum India", stock: 0, maxStock: 10, purchasePrice: "₹45,000", sellingPrice: "₹65,000", margin: "30.8%", status: "Out of Stock", description: "Pure platinum band ring", image: "💎" },
  { id: 5, name: "Kundan Bridal Set", sku: "JK-KB-005", category: "Sets", brand: "Jaipur Artisans", stock: 2, maxStock: 8, purchasePrice: "₹3,20,000", sellingPrice: "₹4,50,000", margin: "28.9%", status: "Low Stock", description: "Complete bridal kundan set", image: "👑" },
  { id: 6, name: "Rose Gold Bracelet", sku: "JK-RB-006", category: "Bracelets", brand: "Golden Touch", stock: 11, maxStock: 20, purchasePrice: "₹28,000", sellingPrice: "₹42,000", margin: "33.3%", status: "In Stock", description: "Rose gold charm bracelet", image: "⭐" },
];
...
export default function InventoryPage() {
  ...
  if (stockFilter !== "all") filtered = filtered.filter(i => i.status === stockFilter);
  if (filterParam === "out" && stockFilter === "all") filtered = filtered.filter(i => i.status === "Out of Stock");
  if (filterParam === "low" && stockFilter === "all") filtered = filtered.filter(i => i.status === "Low Stock");
    ...
    <motion.div className="space-y-6" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        ...
        <CardContent className="p-5">
          <div className="flex flex-col sm:flex-row gap-3 mb-5">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search by name or SKU..." className="pl-9 bg-secondary/50 border-0 h-11 rounded-xl" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
            <Select value={brandFilter} onValueChange={setBrandFilter}>
              <SelectTrigger className="w-[180px] h-11 rounded-xl"><SelectValue placeholder="All Brands" /></SelectTrigger>
              <SelectContent>
            ...
            <Select value={stockFilter} onValueChange={setStockFilter}>
              <SelectTrigger className="w-[160px] h-11 rounded-xl"><SelectValue placeholder="Stock Status" /></SelectTrigger>
              <SelectContent>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm font-sans">
              ...
              <tbody>
                {filtered.map((item) => {
                  const stockPct = Math.round((item.stock / item.maxStock) * 100);
                  return (
                      ...
                      <td className="py-3">
                        <div className="w-24">
                          ...
                          <Progress value={stockPct} className="h-1.5"
                            style={{ '--progress-color': item.status === "Out of Stock" ? "hsl(0, 84%, 60%)" : item.status === "Low Stock" ? "hsl(38, 92%, 50%)" : "hsl(152, 69%, 31%)" } as React.CSSProperties} />
                        </div>
                      </td>
      ...
      <Dialog open={!!viewItem} onOpenChange={() => setViewItem(null)}>
        <DialogContent className="max-w-2xl rounded-2xl">
          <DialogHeader><DialogTitle className="font-display text-xl flex items-center gap-2">{viewItem?.name} <span className={`text-xs px-2.5 py-1 rounded-full ${viewItem ? statusColors[viewItem.status] : ""}`}>{viewItem?.status}</span></DialogTitle></DialogHeader>
          {viewItem && (
            <div className="space-y-4">
              <p className="text-xs text-muted-foreground font-mono">SKU: {viewItem.sku}</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="h-40 rounded-2xl bg-secondary flex items-center justify-center text-6xl">{viewItem.image}</div>
                <div className="space-y-3">
                  <div><p className="text-xs text-muted-foreground">Category</p><p className="font-medium">{viewItem.category}</p></div>
                  <div><p className="text-xs text-muted-foreground">Brand</p><p className="font-medium">{viewItem.brand}</p></div>
                  <div><p className="text-xs text-muted-foreground">Description</p><p className="text-sm">{viewItem.description}</p></div>
                  <div className="grid grid-cols-2 gap-2">
                    <div><p className="text-xs text-muted-foreground">Purchase</p><p className="font-medium">{viewItem.purchasePrice}</p></div>
                    <div><p className="text-xs text-muted-foreground">Selling</p><p className="font-medium">{viewItem.sellingPrice}</p></div>
                  </div>
                </div>
                <div className="space-y-4">
                  <Card className="glass-card rounded-2xl"><CardContent className="p-4 text-center"><p className="text-xs text-muted-foreground font-medium">Current Stock</p><p className="text-4xl font-bold font-display mt-1">{viewItem.stock}</p><p className="text-xs text-accent">of {viewItem.maxStock} max</p><Progress value={(viewItem.stock / viewItem.maxStock) * 100} className="mt-2 h-2" /></CardContent></Card>
                  <Card className="glass-card rounded-2xl"><CardContent className="p-4 text-center"><p className="text-xs text-muted-foreground">Margin</p><p className="text-xl font-bold font-display text-emerald-600">{viewItem.margin}</p></CardContent></Card>
                </div>
              </div>
            </div>
          )}
